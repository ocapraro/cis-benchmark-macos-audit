...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/' && req.method === 'GET') {
    ...
    res.end(data);
  } else if (req.url === '/run-all' && req.method === 'POST') {
    ...
    await writeData(JSON.stringify(tests, null, 2));
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ message: "All checks completed", total: tests.length }));
  } else if (req.url === '/fix-all' && req.method === 'POST') {
    ...
    for (const test of failedTests) {
      ...
      try {
        ...
        console.log(`Fix failed for ${test.title}`);
      }
    }
    
    await writeData(JSON.stringify(tests, null, 2));
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify({ message: "All fixes attempted", total: failedTests.length, fixed }));
